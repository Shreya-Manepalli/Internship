...
def table_2_excel(pdf_path, excel_path, output_path):
    ...
    # Setting up workbook
    write_to_excel("Material No.", 1 + start_point, 1, "result.xlsx", output_sheet_name)
    write_to_excel("NCR-No.", 1 + start_point, 2, "result.xlsx", output_sheet_name)
    write_to_excel("Design Organization Drawing and issue", 1 + start_point, 3, "result.xlsx", output_sheet_name)
    ...
    for z in range(3):
        for zz in range(total_rows):
            write_to_excel(table1[z], zz+2 + start_point, z+1, "result.xlsx", output_sheet_name)


    write_to_excel("Item No.", 1 + start_point, 4, "result.xlsx", output_sheet_name)
    write_to_excel("Defect Type", 1 + start_point, 5, "result.xlsx", output_sheet_name)
    write_to_excel("Charact. No.", 1 + start_point, 6, "result.xlsx", output_sheet_name)
    write_to_excel("Serial numbers affected", 1 + start_point, 7, "result.xlsx", output_sheet_name)
    write_to_excel("Description of Nonconformance", 1 + start_point, 8, "result.xlsx", output_sheet_name)
    ...
    for x in range(total_rows):
        ...
        for y in range(5):
            ...
            if y < 4:
                ...
                # print("=========================")
                # print(table2[x][y])
                write_to_excel(table2[x][y], excel_row + start_point, excel_col, "result.xlsx", output_sheet_name)

            
            elif y == 4:
                str_item_no = str(item_no)
                write_to_excel(str_item_no, excel_row + start_point, 4, "result.xlsx", output_sheet_name)
                item_no = item_no + 1
