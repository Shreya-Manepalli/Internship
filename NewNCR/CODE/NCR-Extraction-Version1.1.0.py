...
def read_pdf_page1_table100(workbook):
...
def print_group_100(table100, start, count, output_path, sheet_name):

    if count != len(table100):
        print("Invalid Input for Serial No.")
    else:
        sheet_name = valid_sheet(sheet_name)
        i = 0
        for i in range(count):
            input_str = str(table100[i])
            write_to_excel(input_str, start, 3, output_path, sheet_name)
            start = start + 1
...
def perform_all_file_actions(pdf_path, output_path):
    ...
    print_group_100(table100, start, count, output_path, sheet_name)
